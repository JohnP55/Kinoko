...
bool KColData::checkSphere(f32 *distOut, EGG::Vector3f *fnrmOut, u16 *flagsOut) {
    ...
    while (*++m_prismIter != 0) {
        u16 prismIdx = parse<u16>(*m_prismIter);
        const KCollisionPrism prism = getPrism(prismIdx);
...
bool KColData::checkSphereSingle(f32 *distOut, EGG::Vector3f *fnrmOut, u16 *flagsOut) {
    ...
    while (*++m_prismIter != 0) {
        ...
        if (checkSphereSingleCollision(prism, distOut, fnrmOut, flagsOut)) {
            return true;
        }
    }
...
/// EQUIVALENT TO checkSphere (0x807C0F00)
bool KColData::checkSphereSingleCollision(const KCollisionPrism &prism, f32 *distOut,
